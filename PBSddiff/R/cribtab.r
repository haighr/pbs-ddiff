...
make.cribtab = function(fdPrefix)
{
	...
	if (fdPrefix=="Nassess") {
		...
		Nrank        = c(1.25, 1.5, 3, 2.5, 1.5, 2, 2, 2, 3, 1.25, 1.5,  3) ## MCMC ranking (subjective ave RH + PJS)
		names(Nrank) = NRrank
		cribtab$rank = rep(NA,30)
		cribtab$rank = Nrank[as.character(cribtab$run)]
#browser();return()
		#cribtab$RG   = c( 1, rep(0,15), rep(NA,14))  ## Reference Group -- in case we need to use a group for model averaging
	...
	if (fdPrefix=="Sassess") {  
		...
		names(Srank) = SRrank
		cribtab$rank = rep(NA,30)
		cribtab$rank = Srank[as.character(cribtab$run)]

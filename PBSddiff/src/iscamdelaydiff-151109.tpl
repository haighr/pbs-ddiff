...
DATA_SECTION
	...
	init_int annual_mean_wt_syr;
	init_int annual_mean_wt_nyr;
	init_vector obs_annual_mean_wt(annual_mean_wt_syr,annual_mean_wt_nyr);
...
FUNCTION initParameters_deldiff
  {
	...
	*/
	...
	h           = theta(2);
	m           = mfexp(theta(3));
	log_avgrec  = theta(1);            // PJS changed this to set ln(R0)=ln(Rbar)
	log_recinit = theta(5);
	/*
	Variance partitioning:		SJDM new code - RF discovered this Sept 13
	-Estimating total variance as = 1/precision
...
FUNCTION calc_objective_function
  {
	...
	*/
	...
	//7) likelihood for mean weight - RF added for P cod
	dvar_vector pred_annual_mean_wt(annual_mean_wt_syr,annual_mean_wt_nyr);
	pred_annual_mean_wt = (annual_mean_wt).sub(annual_mean_wt_syr,annual_mean_wt_nyr);
	
	dvar_vector epsilon_wt = log(pred_annual_mean_wt) - log(obs_annual_mean_wt);
...
FUNCTION void findmsy_dd(double& msy, double& fmsy, double& bmsy)
 {
	/*
	Brute force method to determine msy, fmsy & bmsy
	*/
	static int runNo=0;
	runNo ++;
	int i,j,k;

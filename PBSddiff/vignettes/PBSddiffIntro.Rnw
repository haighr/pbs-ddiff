...
\title{Introduction to PBSddiff}
\author{Rowan Haigh and Chris Grandin}
\maketitle

\section{What is PBSddiff?}

\pbsd{} contains software to facilitate the running of a delay-difference model using the \iscam{} platform.
...
If the delay-difference model is used in future, development will look towards package creation and various styles of GUIs.

To see the development site, navigate to \href{https://github.com/pbs-software/pbs-project}{PBSddiff on GitHub}.
...
At present, there is no User's Guide for PBSddiff. 
...
%
%The R directory \code{../library/PBSddiff/doc} includes a complete User's Guide\\
%\code{PBSddiff-UG.pdf}.
